...
const Sidebar = () => {
  ...
  return (
    <div className={`bg-[#8185B2]/10 h-full p-5 rounded-r-x1 overflow-y-scroll
            ...
            cursor-pointer max-sm:text-sm ${selectedUser?._id === user._id && 'bg-[#282142]/50'}`}>
                ...
                <div className='flex flex-col leading-5'>
                    <p>{user.fullName}</p>
                    {
                        ...
                        ? <span className='text-greeen-400 text-xs'>Online</span>
                        : <span className='text-neutral-400 text-xs'>Offline</span>
                    }

                </div>
                {unseenMessages[user._id] > 0 && <p className='abolute top-4 right-4 text-xs h-5 w-5
